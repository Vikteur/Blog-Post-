...
export function Portfolio() {
  ...
  if (isLoading) {
    return <div className="max-w-7xl mx-auto text-center py-10">
  ...
  return <div className="max-w-7xl mx-auto">
      ...
      <section aria-labelledby="personal-info-heading" className="bg-white rounded-lg shadow-md p-6 mb-8">
        <h2 id="personal-info-heading" className="sr-only">
          Personal Information
        </h2>
        <div className="flex flex-col md:flex-row">
          <div className="md:w-1/3 flex justify-center md:justify-start">
            <div className="w-48 h-48 rounded-full overflow-hidden mb-6 md:mb-0">
              <img src={profile.avatar} alt={`${profile.name}'s profile`} className="w-full h-full object-cover" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {programmingLanguages.map(skill => <div key={skill.name}>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2" role="progressbar" aria-valuenow={skill.level} aria-valuemin={0} aria-valuemax={100} aria-label={`${skill.name} skill level: ${skill.level}%`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projects.map(project => <article key={project.id} className="border rounded-lg overflow-hidden">
              <div className="h-48 bg-gray-200">
                {project.image && <img src={project.image} alt={`${project.title} project screenshot`} className="w-full h-full object-cover" />}
              </div>
              <div className="p-4">
                ...
                {project.url && <a href={project.url} className="flex items-center text-blue-700 hover:text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded-sm" aria-label={`View ${project.title} project details`}>
